...
def ModelXtoCtoY(n_class_attr, pretrained, freeze, num_classes, use_aux, n_attributes, expand_dim,
                 use_relu, use_sigmoid, use_gbsm, expand_gbsm_dim):
    ...
    if n_class_attr == 3:
        ...
        print('n_class_arre != 3')
        if use_gbsm and expand_gbsm_dim:
            print('expanding GBSM dimension')
            model2 = MLP(input_dim=2*n_attributes, num_classes=num_classes, expand_dim=expand_dim)
        else:
            model2 = MLP(input_dim=n_attributes, num_classes=num_classes, expand_dim=expand_dim)
    return End2EndModel(model1, model2, use_relu, use_sigmoid, use_gbsm, expand_gbsm_dim, n_class_attr)
